...
const messages = [];
const configuredApiBase = document.querySelector('meta[name="commerce-api"]')?.content.trim() || "";

// Shared API step: send the request, verify JSON, then normalize errors.
// Keeping this in one function prevents every feature from implementing
// response parsing differently.
async function requestJson(path, options) {
  // "disabled" is injected into static-only packages to avoid calling a
  // non-existent API and attempting to parse the hosting provider's HTML 404.
  if (configuredApiBase === "disabled") {
    throw new Error("Online submissions require the API deployment. Please email info@asrglobalsolutions.com.");
  }
  const apiUrl = configuredApiBase ? `${configuredApiBase.replace(/\/$/, "")}${path}` : path;
  const response = await fetch(apiUrl, options);
  const contentType = response.headers.get("content-type") || "";
  if (!contentType.includes("application/json")) throw new Error("The online service returned an invalid response.");
